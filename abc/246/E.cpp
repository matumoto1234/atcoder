...
void output(vector<vector<int>> vs) {
  for (auto v: vs) {
    std::cout << v << endl;
  }
}
...
struct Flag {
  // 左上, 右上, 右下, 左下
  bool lu, ru, rd, ld;
  Flag(): lu(false), ru(false), rd(false), ld(false) {}
...
int main() {
  ...
  auto dists = make_vector(n, n, INF32);
  auto flgs = make_vector(n, n, Flag());
  dists[ay][ax] = 0;
  deque<pii> q;
  q.emplace_back(ay, ax);
  ...
  while (not q.empty()) {
    ...
    rep(i, 4) {
      int ny = dy[i] + y;
      int nx = dx[i] + x;
      for (; is_inside(ny, nx, n, n); ny += dy[i], nx += dx[i]) {
        if (s[ny][nx] == '#')
          break;
