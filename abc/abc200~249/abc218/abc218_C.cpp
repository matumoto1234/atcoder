...
vector<int> normalize(vector<string> s){
  vector<int> idxs;
  int miny=INF32;
  int minx=INF32;
  range(i,len(s)){
...
bool solve(vector<int> idx, vector<string> t){
  auto idx2=normalize(t);
  debug(idx2);
  return idx==idx2;
}
...
int main() {
  ...
  auto idx=normalize(s);
  // debug(idx);
  bool ans=solve(idx,t);
  t=rot_counterclockwise_2d<string>(t);
  if(solve(idx,t)) ans=true;
  t=rot_counterclockwise_2d<string>(t);
  if(solve(idx,t)) ans=true;
  t=rot_counterclockwise_2d<string>(t);
  if(solve(idx,t)) ans=true;
