...
bool is_same_line(Pll a, Pll b, Pll c) {
  vector<Pll> ps = { a, b, c };

  whole(sort, ps);

  ll dx1 = ps[1].first - ps[0].first;
  ll dx2 = ps[2].first - ps[1].first;
  ll dy1 = ps[1].second - ps[0].second;
  ll dy2 = ps[2].second - ps[1].second;
...
int main() {
  ...
  rep (i, n) {
    rep (j, i) {
      rep (k, j) {
        if (is_same_line(ps[i], ps[j], ps[k])) continue;
        ans++;
      }
    }
  }
