...
int main() {
  ...
  auto dp = make_vector(n + 1, n + 1, mint(0));
  dp[1][1] = 1;
  ...
  range(i, 1, n) {
    ...
    range(j, 1, i + 2) {
      if ( pre > h ) {
        // dp[i+1][j]+=dp[i][j,j+1,...,i-1];
        dp[i + 1][j] = sum.query(1,j);
      }

      if ( pre == h ) {
        // dp[i+1][j]+=dp[i][0,1,...,i-1];
        dp[i + 1][j] = sum.query(0,i+1);
      }

      if ( pre < h ) {
        // dp[i+1][j]+=dp[i][0,1,...,j-1];
        dp[i + 1][j] = sum.query(j,i+1);
      }
    }
  }
