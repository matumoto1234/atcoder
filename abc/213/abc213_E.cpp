...
int main() {
  ...
  deque<P> q;
  q.emplace_back(0,0);
  auto used=make_vector(h,w,INF64);
  used[0][0]=0;
  while(!q.empty()){
    auto [y,x]=q.front();
    q.pop_front();
    ...
    range(i,4){
      ...
      if(!is_range(ny,nx,h,w)) continue;
      if(s[ny][nx]=='.'){
      ...
      if(s[ny][nx]=='#'){
        if(used[ny][nx]==used[y][x]+1){
          range(j,8){
            ...
            if(!is_range(nny,nnx,h,w)) continue;
            if(used[nny][nnx]>used[y][x]+1){
              used[nny][nnx]=used[y][x]+1;
              q.emplace_back(nny,nnx);
            }
          }
        }
        if(used[ny][nx]>used[y][x]+1){
