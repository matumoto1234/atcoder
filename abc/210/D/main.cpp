...
ll h,w,c;
ll solve(vector<vector<ll>> as){
  auto dp1=make_vector(h,w,INF64);
  auto dp2=make_vector(h,w,INF64);

  dp1[h-1][w-1]=as[h-1][w-1];

  rrange(i,h) rrange(j,w){
    if(i-1>=0){
      chmin(dp1[i-1][j],min(dp1[i][j]+c,as[i-1][j]));
      chmin(dp2[i-1][j],dp1[i][j]+c);
    }
    if(j-1>=0){
      chmin(dp1[i][j-1],min(dp1[i][j]+c,as[i][j-1]));
      chmin(dp2[i][j-1],dp1[i][j]+c);
    }
  }

  ll ans=INF64;
  range(i,h) range(j,w){
    chmin(ans,dp2[i][j]+as[i][j]);
  }
...
int main() {
  ...
  cin>>h>>w>>c;
  vector<vector<ll>> as=make_vector(h,w,0LL);
  range(i,h) range(j,w){
    cin>>as[i][j];
  }
