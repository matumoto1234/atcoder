...
vector<pair<int,char>> runlength(string s){
  ...
  for(auto v:s){
    if(res.empty() || res.back().second!=v) res.emplace_back(0,v);
    res.back().first++;
  }
  return res;
}
...
int main() {
  ...
  if(len(vs)){
    if(vs.back().second=='0') vs.emplace_back(0,'1');
    else vs.emplace_back(0,'0');
  }
  ...
  if(vs.front().second=='0') vs.emplace(vs.begin(),0,'1');
  if(vs.back().second=='0') vs.emplace_back(0,'1');


  cumulative_sum<ll> sum(len(vs));
  range(i,len(vs)){
    sum.set(i,vs[i].first);
  }
  ...
  range(i,0,len(vs),2){
    // debug(ans,i);
    if(i+2*k+1>len(vs)) break;
    chmax(ans,sum.query(i,i+2*k+1));
  }
