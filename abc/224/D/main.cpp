...
bool check(const vector<int> &ps) {
  rep (i, 8) {
...
map<vector<int>, P> used;

inline int dfs(int v, int dep, int ans) {
  if (ans <= dep) return ans;
  if (check(ps)) return min(ans, dep);

  for (auto to: G[v]) {
    int i = whole(find, ps, v) - ps.begin();
    int j = whole(find, ps, to) - ps.begin();

    swap(ps[i], ps[j]);

    if (contains(used, ps)) {

      auto [old_dep, memo] = used[ps];

      chmin(ans, memo - old_dep + min(old_dep , dep));
    ...
    used[ps].second = INF32;

    int res = dfs(to, dep + 1, ans);
