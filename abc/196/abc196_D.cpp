...
int main() {
  ...
  int h, w, a, b;
  cin >> h >> w >> a >> b;
  // dp[i][j][k][S]:=(i,j)の一つ手前まで埋めたとき,今までに1*1のマスをk個置いたビット列Sの状態
  auto dp = make_vector(h + 1, w + 1, b + 10, 1 << w, 0);
  dp[0][0][0][0] = 1;
  rep(i, h) rep(j, w) rep(k, b + 1) rep(S, 1 << w) {
    if ( S >> j & 1 ) {
      int nextS = S & ~(1 << j);
      if ( j + 1 < w ) {
        dp[i][j + 1][k][nextS] += dp[i][j][k][S];
      } else {
        dp[i + 1][0][k][nextS] += dp[i][j][k][S];
      }
      continue;
    }
    ...
    int nextS = S;
    if ( j + 1 < w ) {
      dp[i][j + 1][k + 1][nextS] += dp[i][j][k][S];
    } else {
      dp[i + 1][0][k + 1][nextS] += dp[i][j][k][S];
    }
  }
  cout << dp[h][0][b][0] << endl;
}