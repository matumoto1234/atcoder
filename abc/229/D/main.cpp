...
template <typename T1, typename T2> pair<T1, T2> operator+(const pair<T1, T2> &p1, const pair<T1, T2> &p2) { return pair(p1.first + p2.first, p1.second + p2.second); }
template <typename T1, typename T2> pair<T1, T2> operator-(const pair<T1, T2> &p1, const pair<T1, T2> &p2) { return pair(p1.first - p2.first, p1.second - p2.second); }
template <typename T1, typename T2> pair<T1, T2> operator*(const pair<T1, T2> &p1, const pair<T1, T2> &p2) { return pair(p1.first * p2.first, p1.second * p2.second); }
template <typename T1, typename T2> pair<T1, T2> operator/(const pair<T1, T2> &p1, const pair<T1, T2> &p2) { return pair(p1.first / p2.first, p1.second / p2.second); }
template <typename T1, typename T2> pair<T1, T2> operator%(const pair<T1, T2> &p1, const pair<T1, T2> &p2) { return pair(p1.first % p2.first, p1.second % p2.second); }
...
namespace data_structure {
  ...
  struct CumulativeSum {
    vector<T> data;
    CumulativeSum(int n): data(n + 1, pair(0, 0)) {}
    ...
    // [l,r)
    T query(int l, int r) { return pair(data[r].first - data[l].first, data[r].second - data[l].second); }
  };
} // namespace data_structure
...
int main() {
  ...
  auto vs = whole(runlength, s);
  if (vs.front().first == 'X') { vs.insert(vs.begin(), pair('.', 0)); }
  if (vs.back().first == '.') { vs.emplace_back('X', 0); }

  vector<pll> vs2;
  rep(i, 0, len(vs), 2) {
    auto [dot, cnt_dot] = vs[i];
    auto [x, cnt_x] = vs[i + 1];
    vs2.emplace_back(cnt_dot, cnt_x);
  }

  int n = len(vs2);
  CumulativeSum<pll> sum(n);

  rep(i, n) {
    auto [cost, val] = vs2[i];
    sum.set(i, pair(cost, val));
  }
  ...
  rep(i, n) {
    ...
    auto [cost, x_cnt] = sum.query(i + 1, valid);
    x_cnt += sum.query(i, i + 1).second;
    ...
    if (use > 0) {
      ll x = sum.query(valid, n).first;
      x_cnt += min(x, use);
      use -= x;
    }
    if (use > 0) {
      ll x = sum.query(0, i + 1).first;
      x_cnt += min(x, use);
      use -= x;
    }

    chmax(ans, cost + x_cnt);
  }
