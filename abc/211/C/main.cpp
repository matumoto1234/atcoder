...
int main() {
  ...
  auto dp=make_vector(s.size()+1,26,mint(0));
  auto cnt=make_vector(s.size()+1,26,0);
  ...
  rrange(i,s.size()){
    ...
    rrange(j,t.size()-1){
      dp[i][t[j]-'a']+=dp[i+1][t[j]-'a'];
      if(s[i]==t[j]) dp[i][t[j]-'a']+=dp[i+1][t[j+1]-'a'];
    }
  }
  ...
  //   range(j,t.size()){
  //     char c=t[j];
  //     debug(i,j,c,dp[i][t[j]-'a'].val());
  //   }
  // }

  cout<<dp[0]['c'-'a'].val()<<endl;
  return 0;
}