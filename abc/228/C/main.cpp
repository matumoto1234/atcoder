...
int main() {
  ...
  auto ps = make_vector(n, 3, 0);
  rep(i, n) { cin >> ps[i]; }

  vector<int> sum(n, 0);
  rep(i, n) { sum[i] = whole(accumulate, ps[i], 0); }
  ...
  rep(i, n) {
    int idx = whole(upper_bound, sum, sum2[i] + 300) - sum.begin();
    // if(idx!=n){
    //   debug(i,idx,sum[idx],sum2[idx]);
    // }
    cout << (n - idx + 1 <= k ? "Yes" : "No") << newl;
  }
}
