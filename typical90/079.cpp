...
int main() {
  ...
  int h,w;
  cin>>h>>w;
  auto as=make_vector(h,w,0LL);
  auto bs=make_vector(h,w,0LL);

  range(i,h){
    for(auto &a:as[i]) cin>>a;
  }
  range(i,h){
    for(auto &b:bs[i]) cin>>b;
  }
  ...
  ll ans=0;
  range(i,h) range(j,w){
    if(!is_range(i+1,j+1,h,w)) continue;
    int v=bs[i][j]-as[i][j];
    ans+=abs(v);
    range(k,4){
      int ny=i+dy[k];
  ...
  bool valid=true;
  range(i,h) range(j,w){
    if(as[i][j]!=bs[i][j]) valid=false;
  }

  if(valid) cout<<"Yes"<<endl<<ans<<endl;
  else cout<<"No"<<endl;
}