...
int main() {
  ...
  auto bs = make_vector(n + 1, 2, pair(-1, -1));

  auto dp = make_vector(n + 1, 2, cfloat(0));
  dp[0][0] = 1;

  rep (i, n) {
    int a = as[i];

    if (chmax(dp[i + 1][0], dp[i][0])) {
      bs[i + 1][0] = pair(i, 0);
    }
    if (chmax(dp[i + 1][0], dp[i][1] / cfloat(a))) {
      bs[i + 1][0] = pair(i, 1);
    }

    if (chmax(dp[i + 1][1], dp[i][1])) {
      bs[i + 1][1] = pair(i, 1);
    }
    if (chmax(dp[i + 1][1], dp[i][0] * cfloat(a))) {
      bs[i + 1][1] = pair(i, 0);
    }
  }
  ...
  while (true) {
    ...
    if (ni == -1 or nj == -1) break;

    ans.emplace_back(j != nj);
