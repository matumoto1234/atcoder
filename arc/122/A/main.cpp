...
int main() {
	...
	auto pat=make_vector(n+1,2,mint(0));
	auto dp=make_vector(n+1,2,mint(0));
	...
	range(i,1,n){
		pat[i][0]+=pat[i-1][0]+pat[i-1][1];
		pat[i][1]+=pat[i-1][0];

		dp[i][0]+=dp[i-1][0]+a[i]*pat[i-1][0] + dp[i-1][1]+a[i]*pat[i-1][1];
		dp[i][1]+=dp[i-1][0]-a[i]*pat[i-1][0];
	}

	// range(i,0,n){
	// 	cerr<<pat[i][0].val()<<" "<<pat[i][1].val()<<endl;
	// }
