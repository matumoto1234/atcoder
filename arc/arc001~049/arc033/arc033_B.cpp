...
int main() {
  ...
  vector<int> inter;
  set_intersection(as.begin(),as.end(),bs.begin(),bs.end(),inserter(inter,inter.end()));

  vector<int> uni;
  set_union(as.begin(),as.end(),bs.begin(),bs.end(),inserter(uni,uni.end()));

  cout<<double(len(inter))/len(uni)<<endl;
}