...
int find_L(const vector<int> &section) {
  rep (i, len(section)) {
...
int find_R(const vector<int> &section) {
  rrep(i, len(section)) {
    if (section[i] == 1) return i;
  }
  return -1;
}
...
int main() {
  ...
  sep_idxs.emplace_back(0);

  rep (i, 1, len(vs)) {
    if (vs[i - 1] == 0 and vs[i]) { sep_idxs.emplace_back(i); }
  }

  sep_idxs.emplace_back(len(vs));

  auto left_sum = [&](int idx) -> ll {
    ll space_cnt = 0;
    ll sum = 0;

    rrep(i, idx) {
      int nx = i + 1 == len(vs) ? l : comp[i + 1];
      int x = comp[i];
      space_cnt += nx - x;

      if (vs[i] == 1) sum += space_cnt;
    }
    return sum;
  };

  auto right_sum = [&](int idx) -> ll {
    ll space_cnt = 0;
    ll sum = 0;
    ...
    rep(i, idx, len(vs)) {
      ...
      if (vs[i] == 0) sum += space_cnt;
    }
    return sum;
  };

  auto which = [&](const vector<int> &section) -> ll {
    int l_idx = find_L(section);
    int r_idx = find_R(section);

    debug(section);
    debug(l_idx, r_idx);
    debug(left_sum(l_idx), right_sum(r_idx));

    return max(left_sum(l_idx),right_sum(r_idx));
  };
  ...
  rep (i, len(sep_idxs) - 1) {
    int l = sep_idxs[i];
    int r = sep_idxs[i + 1];

    debug(which(sl(vs,l,r)));
    ans += which(sl(vs, l, r));
  }
