...
int main() {
  ...
  auto cnt1 = counter(s1);
  auto cnt2 = counter(s2);
  auto cnt3 = counter(s3);
  ...
  rep (i, 26) {
    ...
    G.add_edge(v1, alpha, cnt1[c]);
    G.add_edge(v2, alpha, cnt2[c]);

    G.add_edge(alpha, sink, cnt3[c]);
  }

  if (len(s3) == G.flow(src, sink)) {
    cout << "YES" << endl;
  } else {
    cout << "NO" << endl;
  }
